...
use crate::screen::{Coordinate, GameContent, ScreenBuffer, send_events, KeyEventQueue};
...
#[derive(PartialEq, Clone, Debug)]
pub struct Player {
    pub left_key: KeyEvent,
    pub right_key: KeyEvent,
    pub up_key: KeyEvent,
    pub down_key: KeyEvent,
    pub snake: Snake,
...
impl Player {
    ...
    pub fn update_snake_direction(
        ...
        if is_four_key_steering {
            self.update_direction_four_keys(key_event);
        } else {
            self.update_direction_two_keys(key_event);
        }
    }

    fn update_direction_four_keys(&mut self, key_event: KeyEvent) {
    ...
    fn update_direction_two_keys(&mut self, key_event: KeyEvent) {
